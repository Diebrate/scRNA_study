...
def loss_unbalanced(a, b, costm, reg, reg1, reg2, sink=True, single=True):
    ...
    if sink:
        ...
        return loss(tmap_ab, a, b, costm, reg, reg1, reg2)
...
def loss_unbalanced_partial(a, b, costm, reg, reg1, reg2, sink=True):
    ...
    if sink:
        ...
        return loss(tmap_ab, a_sub, b_sub, get_sub(costm, ind_a, ind_b), reg, reg1, reg2)
...
def loss_unbalanced_cont(x0, x1, reg, reg1, reg2, dim, sink=True):
    ...
    if sink:
        ...
        tmap_ab = (a, b, costm_ab, reg, reg1, reg2)
...
def loss_unbalanced_all_local(probs, costm, reg, reg1, reg2, sink=True, win_size=1, weight=None, partial=False):
    ...
    if win_size > 1:
        ...
        if partial:
            res = np.zeros(T)
            for t in range(T):
                res[t] = loss_unbalanced_partial(probs[t, ], probs[t + 1, ], costm, reg, reg1, reg2, sink=sink)
        else:
            res = loss_unbalanced(probs[:T, ].T, probs[1:, ].T, costm, reg, reg1, reg2, sink=sink, single=False)
    return res
